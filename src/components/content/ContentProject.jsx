...
export function RetangleProjects({nome, img, resum, idProjeto}) {
    return(
        <div className="flex flex-col items-center justify-center w-full h-550" >
            <div className="flex flex-col items-center justify-center relative w-full max-w-900 h-550 overflow-hidden group cursor-cursoClick border-2 border-black/10 bg-white p-6 " >
                <div className="flex items-center justify-between w-full h-550 pb-6" >
                    <Heading as="h4" size="small" color="black" className="font-semibold" >{nome}</Heading>
                    <span className="w-full text-black/70  font-roobert text-end" >{idProjeto}</span>
                </div>
                <div className="relative w-full h-550 overflow-hidden" >
                    <Image src={`/image/${img}`} width={1200} height={150} alt="Projeto" className="w-full h-550" />
                ...
                <div className="w-full flex flex-col items-start justify-center gap-4 pb-6 pt-6" >
                    <Paragraph size="small" className={"text-black/70"}>{resum}</Paragraph> 
                    <LinkCustomBlack/>
                </div>
            </div>
        </div>
